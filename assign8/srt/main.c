...
void *draw_pixels(void *thread_num_arg);
...
int nthreads;
float *pixels[3]; //array of all pixel RGB values
scene_t scene;
double pixel_dx;
double pixel_dy;
double subsample_dx;
...
main( int argc, char **argv )
{
    ...
    nthreads = min(nthreads, width); //minimum of 1 pixel width, no need for additional threads
    ...
    for (int color = 0; color < 3; color++) {
        pixels[color] = malloc((width * height) * sizeof(float*));
        
        if (pixels[color] == NULL)
        {
            fprintf(stderr, "Error allocating memory for %d pixels\n", width * height);
            exit(1);
        }
    }
    ...
    if (threads == NULL)
    {
        fprintf(stderr, "Error allocating memory for thread array\n");
        exit(1);
    }
    
    for (int thread_num = 0; thread_num < nthreads; thread_num++)
    {
        //create separate thread number variable for each thread to prevent race conditions
        int *thread_num_arg = malloc(sizeof(int));
        
        if (thread_num_arg == NULL) {
            fprintf(stderr, "Error allocating memory for thread number argument %d\n", thread_num);
            exit(1);
        }
        
        *thread_num_arg = thread_num;
        if (pthread_create(&threads[thread_num], NULL, draw_pixels, thread_num_arg) != 0)
        {
    ...
    for (int thread_num = 0; thread_num < nthreads; thread_num++)
    {
        if (pthread_join(threads[thread_num], NULL) != 0)
        {
...
{    
    ...
    int section_width = width / nthreads;
    int start_x = section_width * thread_num;
    int end_x;
    
    if (thread_num == nthreads - 1)
        end_x = width;
    else
        end_x = section_width * (thread_num + 1);
    ...
    for( int px=start_x; px<end_x; ++px )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            pixels[0][height*px + py] = gamma( pixel_color[0] ) * max_color;
            pixels[1][height*px + py] = gamma( pixel_color[1] ) * max_color;
            pixels[2][height*px + py] = gamma( pixel_color[2] ) * max_color;

            // enforce caps, replace with real gamma
            for( int i=0; i<3; i++)
                pixels[i][height*px + py] = max( min(pixels[i][height*px + py], 255), 0);
        }
    }
